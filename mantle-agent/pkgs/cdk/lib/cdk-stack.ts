...
export class CdkStack extends cdk.Stack {
	...
	constructor(scope: Construct, id: string, props?: cdk.StackProps) {
		...
		if (fs.existsSync(envPath)) {
			envVars = dotenv.parse(fs.readFileSync(envPath)) as Record<
				string,
				string
			>;
		} else {
		...
		// ── 必須パラメータの SSM 登録 ──
		const requiredParams: Record<RequiredKey, ssm.StringParameter> =
			{} as Record<RequiredKey, ssm.StringParameter>;
		...
		const lambdaEnv: Record<string, string> = {
			GOOGLE_GENERATIVE_AI_API_KEY:
				requiredParams["GOOGLE_GENERATIVE_AI_API_KEY"].stringValue,
